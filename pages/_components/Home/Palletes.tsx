  ...
  return (
    <div className="w-[32vw] h-[35vh] rounded-3xl flex flex-row shadow-2xl">
      {color && color.map((col, index) => (
        <div
          key={index}
          className={`h-full w-[8vw]  bg-[${col}] ${
            index === 0 ? "rounded-l-3xl" : index === color.length - 1 ? "rounded-r-3xl" : ""
          } cursor-pointer`}
          style={{ backgroundColor: col, height: "100%", width: "8vw" }}
          onClick={() => copyColor(col)}
        >
          <div className="flex justify-center items-center opacity-0  h-full hover:opacity-100 duration-300 transition-all text-xl font-bold">
